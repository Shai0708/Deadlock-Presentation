...
const SlideRenderer: React.FC<Props> = ({ slide }) => {
  ...
  const getIcon = (iconName?: string, animate: boolean = false) => {
    ...
    switch(iconName) {
      ...
      case 'unlock': return <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 11V7a4 4 118 0m-4 8v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2z"/></svg>;
      case 'bundle': return <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>;
      case 'shield-off': return <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/></svg>;
      case 'sort': return <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12"/></svg>;
      case 'brain': return <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.989-2.386l-.548-.547z"/></svg>;
      case 'check-circle': return <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>;
      case 'cross-circle': return <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>;
      case 'terminate': return <span className="text-xl">🛑</span>;
      case 'transfer': return <span className="text-xl">🔄</span>;
      case 'history': return <span className="text-xl">⏪</span>;
      default: return null;
    }
  };
          ...
          {slide.points?.map((p, idx) => (
            ...
            >
              ...
              <div className="overflow-hidden">
                ...
                <p className={`text-[8px] font-bold uppercase tracking-widest mt-0.5 transition-opacity ${activePoint === idx ? 'text-blue-100 opacity-100' : 'text-slate-400 opacity-0 group-hover:opacity-100'}`}>See Technical Meaning</p>
              </div>
            </button>
          ))}
        </div>
  ...
  const renderSimulation = () => {
    const activeExplainer = slide.points?.find(p => 
      simMode === 'deadlock' ? p.label.includes('Deadlock') : p.label.includes('Safe')
    );
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 h-full gap-12 py-6 max-w-6xl mx-auto items-center animate-in fade-in duration-1000 relative w-full">
        ...
        <div className="space-y-6 flex flex-col justify-center h-full">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-md animate-in slide-in-from-bottom duration-500">
            <div className="flex items-center gap-3 mb-3">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-white ${simMode === 'deadlock' ? 'bg-rose-500' : 'bg-emerald-500'}`}>
                {getIcon(activeExplainer?.icon)}
              </div>
              <h4 className={`font-black text-sm uppercase tracking-wider ${simMode === 'deadlock' ? 'text-rose-600' : 'text-emerald-600'}`}>
                Simulation Logic: {simMode === 'deadlock' ? 'Cyclic Dependency' : 'Safe Ordering'}
              </h4>
            </div>
            <p className="text-slate-600 font-medium leading-relaxed italic">
              {activeExplainer?.description}
            </p>
          </div>
          ...
          <div className="pt-4 flex gap-4">
            <button 
              ...
              className={`flex-1 px-6 py-4 rounded-2xl font-black transition-all ${simMode === 'deadlock' ? 'bg-rose-600 text-white shadow-xl shadow-rose-200 scale-105' : 'bg-white text-slate-400 border border-slate-100'}`}
            >
              Classic Cycle
            </button>
            <button 
              onClick={() => setSimMode('safe')}
              className={`flex-1 px-6 py-4 rounded-2xl font-black transition-all ${simMode === 'safe' ? 'bg-emerald-600 text-white shadow-xl shadow-emerald-200 scale-105' : 'bg-white text-slate-400 border border-slate-100'}`}
            >
              Linear Ordering
            </button>
          </div>
        ...
        <div className={`flex items-center justify-center bg-white p-10 rounded-[48px] border ${simMode === 'safe' ? 'border-emerald-200 shadow-emerald-500/10' : 'border-slate-100 shadow-xl'} backdrop-blur-sm h-fit transition-all duration-700`}>
          <RAGVisualizer mode={simMode === 'deadlock' ? 'interactive-rag' : 'safe'} step={3} />
        </div>
      </div>
    );
  };
  ...
  const renderSplit = () => (
    <div className="grid grid-cols-1 lg:grid-cols-2 h-full gap-12 py-6 max-w-6xl mx-auto items-center animate-in fade-in duration-1000 relative w-full">
      ...
      <div className="flex items-center justify-center bg-white p-10 rounded-[48px] border border-slate-100 shadow-xl backdrop-blur-sm h-fit">
        <RAGVisualizer mode={slide.visualId as any || 'deadlock'} step={slide.visualStep} />
      </div>
    </div>
  );
